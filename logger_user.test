...
foreach (array('logger_user.common.inc') as $file2require) {
	...
	if (! $is_found) {
		drupal_set_message($file2require . ' is not found.', 'error');
	}
}

// require_once sprintf('%s/%s/%s', DRUPAL_ROOT, drupal_get_path('module', 'logger_user'), 'logger_user.common.inc');

/**
 * @file
 * Tests for logger_user.module.
 */

/**
 * Tests logging messages to the database.
 */
class LoggerUserUnitTestCase extends DrupalUnitTestCase {
	public static function getInfo() {
		// Note: getInfo() strings should not be translated.
		return array(
			'name' => 'Logger-User unit tests',
			'description' => 'Unit testing of Logger-User class.',
      'group' => 'Loggeruser',
		);
	}

  function setUp() {
    parent::setUp(array('logger_user'));
    // parent::setUp('logger_user');
  }

	// function testeight() {
	// 	$this->assertEqual(8, logger_user_test1());
	// }
  ...
  /**
	 ...
	 */
	function testconst() {
		$this->assertEqual('logger_user_users', LoggerUser::DB_NAME);
	}

  /**
	 * Tests LoggerUser::select_columns().
	 */
	function testselect_columns() {
		static $func = 'select_columns';
		$hsres = array(
			'concise' => NULL,
			'standard' => NULL,
			'all' => NULL,
		);
		$ncol = $hsres;	// Initialized.

		$c = LoggerUser::select_columns('concise', 'colnames');
		$message = sprintf('%s(,%s)[u][0]', $func, 'colnames');
		$this->assertEqual('uid', $c['u'][0], $message);

		foreach (array_keys($hsres) as $k) {
			$hsres[$k] = LoggerUser::select_columns($k, 'header');
		}
		// // No stdout/stderr is allowed, apparently...
		// dsm($ncol);
		// drupal_set_message('ncol=' . var_export($ncol, TRUE), 'error');
		// // So, a way is to include it in the message!
		// $message = 'ncol=' . var_export($ncol, TRUE);

		$message = $func;
		$this->assertEqual(7, count($hsres['concise']), 'concise-size' . $message);

		$this->assertTrue(
			count($hsres['concise']) <= count($hsres['standard']),
			'(concise <= standard) ' . $message
		);
		$this->assertTrue(
			count($hsres['standard']) < count($hsres['all']),
			'(standard < all) ' . $message
		);

		$ar = array_keys($hsres['standard'][0]);
		$this->assertEqual(array('data', 'field', 'sort'), sort($ar));

		$this->assertEqual(
			'uid',
			strtolower($hsres['concise'][0]['data']),
			'(concise - uid) ' . $message
		);
	}	// function testselect_columns() {

  /**
	 * Tests LoggerUser::tablealiases().
	 */
	function testtablealiases() {
    $result = LoggerUser::tablealiases('self');
		// Note: Test output messages should not be translated.
		$message = 'tablealiases("self").';
		$this->assertEqual(1, count($result), 'Size ' . $message);
		$this->assertEqual('l', $result[0], 'Content ' . $message);
 ...
 * Tests logging messages to the database.
 */
class LoggerUserTestCase extends DrupalWebTestCase {
  ...
  public static function getInfo() {
    return array(
      'name' => 'Logger-User functionality',
      'description' => 'Generate events and verify logger_user entries; verify user access to log reports based on persmissions.',
      'group' => 'Loggeruser',
    );
  }
